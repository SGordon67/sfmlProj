...
float degreesToRadians(float degrees) { return (degrees * (M_PI / 180.f)); }
        ...
        const sf::Vector2f &otherPosition)
{
    ...
    if (std::abs(delta.x) > worldWidth / 2.0f)
        ClosestPosition.x =
            otherPosition.x + (delta.x > 0 ? (-worldWidth) : worldWidth);
    if (std::abs(delta.y) > worldHeight / 2.0f)
        ClosestPosition.y =
            otherPosition.y + (delta.y > 0 ? (-worldHeight) : worldHeight);
        ...
        float radius)
{
    ...
    // float wrapYThreshold = (std::abs(worldHeight - (viewHeight / 2.f)) +
    // size.y);
    ...
    if (nearLeft)
        dupPositions.push_back(position +
                sf::Vector2f{static_cast<float>(worldWidth), 0});
    if (nearRight)
        dupPositions.push_back(position +
                sf::Vector2f{-static_cast<float>(worldWidth), 0});
    if (nearTop)
        dupPositions.push_back(position +
                sf::Vector2f{0, static_cast<float>(worldHeight)});
    if (nearBottom)
        dupPositions.push_back(position +
                sf::Vector2f{0, -static_cast<float>(worldHeight)});
    ...
    if (nearLeft && nearTop)
        dupPositions.push_back(position +
                sf::Vector2f{static_cast<float>(worldWidth),
                static_cast<float>(worldHeight)});
        ...
        float restitution = 1.0f, float friction = 0.5f)
{
    ...
    sf::Vector2f otherClosesetPosition = getClosestWrapPosition(
            mainObject->getPosition(), otherObject->getPosition());
    ...
    float I1 = 0.05f * mainObject->getMass() * mainObject->getRadius() *
        mainObject->getRadius();
    float I2 = 0.05f * otherObject->getMass() * otherObject->getRadius() *
    ...
    mainObject->setVelocity(mainObject->getVelocity() +
            impulse / mainObject->getMass());
    otherObject->setVelocity(otherObject->getVelocity() -
            impulse / otherObject->getMass());
    ...
    // Calculate moment of inertia
    I1 = 0.05f * mainObject->getMass() * mainObject->getRadius() *
        mainObject->getRadius();
    I2 = 0.05f * otherObject->getMass() * otherObject->getRadius() *
        otherObject->getRadius();
    ...
    float frictionImpulseScalar =
        -relativeVelocityTangent * friction /
        ((1.0f / mainObject->getMass()) + (1.0f / otherObject->getMass()) +
    ...
    mainObject->setVelocity(mainObject->getVelocity() +
            frictionImpulse / mainObject->getMass());
    otherObject->setVelocity(otherObject->getVelocity() -
            frictionImpulse / otherObject->getMass());
    ...
    mainObject->setAngularVelocity(mainObject->getAngularVelocity() +
            frictionAngularImpulse1);
    float frictionTorque2 =
        -(r2.x * frictionImpulse.y - r2.y * frictionImpulse.x);
    float frictionAngularImpulse2 = frictionTorque2 / I2;
    otherObject->setAngularVelocity(otherObject->getAngularVelocity() +
        ...
        QuadTree &quadTree)
{
    ...
    for (auto &obj : physicalObjects)
    {
        ...
        sf::Vector2f objClosesetPosition =
            getClosestWrapPosition(player->getPosition(), obj->getPosition());
                ...
                (std::sqrt(viewWidth * viewWidth + viewHeight * viewHeight) / 2))
        {
            ...
            {
                if (obj->getObjectID() == other->getObjectID())
                    continue;
        ...
        const std::shared_ptr<Player> &player)
{
    ...
    // physicalObjects.push_back(std::make_shared<Ball>(Ball(sf::Vector2f(0.1,
    // 0.1))));
    // physicalObjects.push_back(std::make_shared<Ball>(Ball(sf::Vector2f(worldWidth
    // - 2*objectRadius, 0.1))));
    //
    // // two objects touching eachother near the middle of the board
    // physicalObjects.push_back(std::make_shared<Ball>(Ball(sf::Vector2f(500,
    // 200))));
    // physicalObjects.push_back(std::make_shared<Ball>(Ball(sf::Vector2f(500 +
    // objectRadius, 200.5))));
    //
    // // two objects near above player start to test collision rotation
    // physicalObjects.push_back(std::make_shared<Ball>(Ball(sf::Vector2f(900,
    // 400))));
    ...
    // Hazards
    // create hazards
    // auto h0 = std::make_shared<Spikey>(Spikey(sf::Vector2f(distX(rng),
    ...
    for (int i = 0; i < numEnemies; i++)
    {
        ...
        // no longer needed for physical hazards, will be needed for visual hazards
        // (non-physical) hazardousObjects.push_back(eR);
    }
}
...
int main()
{
    ...
    resolutionManager.setResolution(windowWidth, windowHeight);
                ...
                if (mouseButton->button == sf::Mouse::Button::Left)
                {
                    ...
                    switch(currentState)
                    {
                        case(GameState::MainMenu):
                            {
                                if (mainMenu.isStartButtonHovered(mousePos))
                                {
                                    resetGame();
                                    currentState = GameState::Playing;
                                } else if(mainMenu.isSettingsButtonHovered(mousePos))
                                {
                        ...
                        case(GameState::Settings):
                            {
                                if(settingsScreen.isBackHovered(mousePos))
                                {
                                    currentState = GameState::MainMenu;
                                }
                                int resIndex = settingsScreen.getHoveredResolution(mousePos);
                                if(resIndex >= 0)
                                {
                        ...
                        case(GameState::GameOver):
                            {
                                if (gameOverScreen.isRestartButtonHovered(mousePos))
                                {
                                    resetGame();
                                    currentState = GameState::Playing;
                                } else if (gameOverScreen.isMenuButtonHovered(mousePos))
                                {
                                    currentState = GameState::MainMenu;
                                }
                                break;
